...
class SignUpStudentStep2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      firstName: '',
  ...
  render() {
    ...
    const { firstName, lastName, mobilePhone, password, confirm, checked, error } = this.state;
      ...
      <Container>        
        ...
        <Content contentContainerStyle={styles.container}>
          ...
          <ValidationInput
            ...
            value={mobilePhone}
            onChange={(mobilePhone) => this.setState({mobilePhone})}
  ...
  checkValidation() {
    const { firstName, lastName, mobilePhone, password, confirm, checked } = this.state;
    const error = {
      mobilePhone: mobilePhone.length < 11,
...
export default connect(state => ({
  temp: state.temp,
}), mapDispatchToProps)(SignUpStudentStep2);
